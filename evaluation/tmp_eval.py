...
from T20_eval import main as tmain
...
def raw_mouse_t20():
    lab_mouse_t20_dirpath = '/data/home/waitma/antibody_proj/antidiff/data/lab_data/sample_t20_mouse_score.csv'
...
def exp_human_t20():
    lab_exper_t20_dirpath = '/data/home/waitma/antibody_proj/antidiff/data/lab_data/sample_t20_score.csv'
...
def main(root_path):
    ...
    logger('Experimental_H_score_improvement: {}'.format(exp_h_t20 - mouse_h_t20))
    logger('Experimental_L_score_improvement: {}'.format(exp_l_t20 - mouse_l_t20))

    logger('Sample_H_score_improvement: {}'.format(sample_h_t20 - mouse_h_t20))
    logger('Sample_L_score_improvement: {}'.format(sample_l_t20 - mouse_l_t20))

    # Oasis
    exec_path = '/data/home/waitma/anaconda3/envs/biophi/bin/biophi'
    input_fa_path = os.path.join(os.path.dirname(root_path), 'sample_identity.fa')
    oasis_db = '/data/home/waitma/antibody_proj/BioPhi-2021-publication/OASis_9mers_v1.db'
    ...
    lab_mouse_dirpath = '/data/home/waitma/antibody_proj/antidiff/data/lab_data/lab_mouse_identity_oasis.xlsx'
    lab_exper_dirpath = '/data/home/waitma/antibody_proj/antidiff/data/lab_data/lab_identity_oasis.xlsx'
    ...
    logger('Lab oasis medium improve: {}'.format(labexp_oasis_curve['50%'].mean() - mouse_oasis_curve['50%'].mean()))
    logger('Sample oasis medium improve: {}'.format(sample_oasis_curve['50%'].mean() - mouse_oasis_curve['50%'].mean()))

    # # Identity.
    # identity_ratio = cal_group_fr_germline_identity(sample_t20_df, scheme='imgt')
    # print('H germline identity: {}'.format(np.array(identity_ratio)[:, 0].mean()))
    # print('L germline identity: {}'.format(np.array(identity_ratio)[:, 1].mean()))

    # Preservation
    sample_df = pd.read_csv(root_path)
    sample_human_df = sample_df[sample_df['Specific'] == 'humanization'].reset_index(drop=True)

    lab_human_fpath = '/data/home/waitma/antibody_proj/antidiff/data/lab_data/humanization_pair_data_filter.csv'
    lab_df = pd.read_csv(lab_human_fpath)
    lab_human_df = lab_df[lab_df['type'] == 'humanized'].reset_index(drop=True)
    lab_mouse_df = lab_df[lab_df['type'] != 'humanized'].reset_index(drop=True)
    ...
    logger('H Experimental all Preservation: {}'.format(np.array(exp_preservation_all_list)[:, 0].mean()))
    logger('L Experimental all Preservation: {}'.format(np.array(exp_preservation_all_list)[:, 1].mean()))
    print('--------------------------------------------')
    logger('H Experimental vernier Preservation: {}'.format(np.array(exp_preservation_vernier_list)[:, 0].mean()))
    logger('L Experimental vernier Preservation: {}'.format(np.array(exp_preservation_vernier_list)[:, 1].mean()))
    ...
    logger('H Sample all Preservation: {}'.format(np.array(sample_preservation_all_list)[:, 0].mean()))
    logger('L Sample all Preservation: {}'.format(np.array(sample_preservation_all_list)[:, 1].mean()))
    print('---------------------------------------------')
    logger('H Sample vernier Preservation: {}'.format(np.array(sample_preservation_vernier_list)[:, 0].mean()))
    logger('L Sample vernier Preservation: {}'.format(np.array(sample_preservation_vernier_list)[:, 1].mean()))
    ...
    logger('H Vernier Mutation precision: {}'.format(np.array(vernier_ratio_list_h).mean()))
    logger('L Vernier Mutation precision: {}'.format(np.array(vernier_ratio_list_l).mean()))
    ...
    logger('H Vernier Mutation precision: {}'.format(np.array(fr_ratio_list_h).mean()))
    logger('L Vernier Mutation precision: {}'.format(np.array(fr_ratio_list_l).mean()))


if __name__ == '__main__':

    # Define a root path, which is the csv file of the sample results.
    root_path = '/apdcephfs/share_1364275/waitma/anti_proj/log_v6/' \
                'v006_pair_not_pretrain_filter_split_2024_01_16__21_20_29/2023_shuffle_lab_not_equal_2024_01_22__09_40_39/sample_humanization_result.csv'
    # root_path = '/apdcephfs/share_1364275/waitma/anti_proj/log_v3/' \
    #             'v004_pair_not_pretrain_test_split_2023_12_25__16_05_03/' \
    #             '2023_shuffle_lab_2023_12_26__16_12_15/sample_humanization_result.csv'
    main(root_path)
